...
export default function CollectionsPage() {
  ...
  const [sortOption, setSortOption] = useState<
    "price-low" | "price-high" | "year-new" | "year-old"
  >("price-low");
    ...
    const fetchCars = async () => {
      try {
        ...
        const uniqueModels = Array.from(
          new Set(data.map((car: Car) => car.model))
        ) as string[];
  ...
  useEffect(() => {
    ...
    if (selectedColors.length > 0) {
      result = result.filter((car) =>
        car.color.some((color) => selectedColors.includes(color))
      );
    }
              ...
              <div className="flex flex-wrap gap-2">
                {colors.map((color) => (
                  <div
                    ...
                    className={`w-8 h-8 rounded-full cursor-pointer border-2 ${selectedColors.includes(color) ? "border-blue-500" : "border-gray-300"}`}
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex justify-between items-center">
                        <div>
                          ...
                          {car.forRent && (
                            <p className="text-sm">${car.rentalPrice}/day</p>
                          )}
                        </div>
                        <div className="text-sm">
                          {car.forRent && (
                            <span
                              className={`px-2 py-1 rounded ${car.isAvailable ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
                            >
                          ...
                          {car.forSale && (
                            <span
                              className={`px-2 py-1 rounded ${car.inStock ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
                            >
